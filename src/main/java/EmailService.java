...
public class EmailService {
    ...
    private static String createHtmlReport(List<CryptoData> cryptoList) {
        ...
        html.append("<p>📸 Portfolio screenshot is attached to this email</p>");
    ...
    /**
     * Format AI analysis for email display (simplified version)
     */
    private static String formatAiAnalysisForEmail(String analysis) {
        ...
        try {
            ...
            StringBuilder html = new StringBuilder();
            html.append("<div style='background-color: #F8F9FA; padding: 12px; border-radius: 6px; border-left: 3px solid #1976D2; font-size: 12px;'>");
            ...
            for (String line : lines) {
                ...
                if (line.toUpperCase().contains("RECOMMENDATION") || 
                    line.toUpperCase().contains("ACTION") ||
                    line.toUpperCase().contains("POSITION")) {
                    
                    html.append("<div style='margin-bottom: 8px;'>");
                    html.append("<strong style='color: #1976D2;'>💡 Recommendation:</strong>");
                    foundRecommendation = true;
                    continue;
                }
                
                if (line.toUpperCase().contains("SUMMARY") || 
                    line.toUpperCase().contains("ANALYSIS")) {
                    ...
                    html.append("<div style='margin-bottom: 8px;'>");
                    html.append("<strong style='color: #1976D2;'>📊 Summary:</strong>");
                    foundSummary = true;
                    continue;
                }
                
                // Process content lines
                if (foundRecommendation || foundSummary) {
                    if (line.startsWith("•") || line.startsWith("-")) {
                        // Bullet points
                        html.append("<div style='margin: 4px 0; padding-left: 12px;'>");
                        html.append("▸ ").append(formatEmailContentLine(line.substring(1).trim()));
                        html.append("</div>");
                    } else if (!line.matches("^[🔥📊💰📈🎯🔍][A-Z ]+.*") && 
                              !line.matches("^[A-Z][A-Z ]{10,}.*")) {
                        // Regular content (skip section headers)
            ...
            // If no structured content found, show first few lines
            if (!foundRecommendation && !foundSummary && lines.length > 0) {
                html.append("<div>");
                html.append("<strong style='color: #1976D2;'>🤖 AI Insight:</strong>");
                int count = 0;
                for (String line : lines) {
                    line = line.trim();
                    if (!line.isEmpty() && !line.contains("═══") && !line.contains("---") && count < 3) {
                        html.append("<div style='margin: 4px 0;'>");
                        html.append(formatEmailContentLine(line));
                        html.append("</div>");
                        count++;
    ...
    /**
     * Format content lines for email (simplified version)
     */
